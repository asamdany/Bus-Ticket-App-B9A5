  ...
  singleSeat.addEventListener("click", function () {
    if (clickCount < 4) {
      if (singleSeat.classList.contains("selectedSeat")) {
        ...
        clickCount--;
        let promoApplyButtonHolder =
          document.getElementById("promoApplyButton");
        ...
        let readyDiv =
          `<div id="` +
          seatName +
          `" class="flex flex-row justify-between pb-4"><div class="text-primaryheadingColor">` +
          seatName +
          `</div><div class="text-primaryheadingColor">Economoy</div><div class="text-primaryheadingColor">550</div></div>`;
...
promoApplyButton.addEventListener("click", function () {
  ...
  if (promoCode.value === "NEW15") {
    ...
    old_grand_total_holder.innerText =
      initialTotal - (parseInt(initialTotal) * 15) / 100;
    ...
    let readyDiv =
      `<div id="discountBar" class="flex flex-row justify-between pb-4"><div class="text-primaryheadingColor">Discount</div><div class="text-primaryheadingColor">BDT -` +
      (parseInt(initialTotal) * 15) / 100 +
      `</div></div>`;
    ...
    let initialTotal = document.getElementById("initial-total").innerText;
    old_grand_total_holder.innerText =
      initialTotal - (parseInt(initialTotal) * 20) / 100;
    let promo_success = document.getElementsByClassName("promo-success");
    promo_success[0].classList.remove("hidden");
    document.getElementById("promo-block").classList.add("hidden");
    let readyDiv =
      `<div id="discountBar" class="flex flex-row justify-between pb-4"><div class="text-primaryheadingColor">Discount</div><div class="text-primaryheadingColor">BDT -` +
      (parseInt(initialTotal) * 20) / 100 +
      `</div></div>`;
    document.querySelector(".add-discount-list").innerHTML += readyDiv;
  } else {
